    ...
    decollide.repetitions = 10, ...) 
{
    if (length(x) > 1 | length(y) > 1) {
        ...
        if (!is.null(maxlength)) 
            for (i in 1:length(x)) text[i] = qqBaseX::trim(qqBaseX::textbreaker(text[i], 
                maxlength = maxlength))
        if (decollide) {
            print(paste0("x=", x, "\ny=", y, "\ntext=", text))
            d = qqBaseX::decollide(x, y, text, repetitions = decollide.repetitions)
            x = d[, "x"]
            y = d[, "y"]
        }
    ...
    if (!is.null(pos)) 
        if (!is.numeric(pos)) 
            ...
            pos = NA
    if (!is.null(maxlength)) 
        text = qqBaseX::trim(qqBaseX::textbreaker(text, maxlength = maxlength))
    if (decollide) {
        d = qqBaseX::decollide(x, y, text, repetitions = decollide.repetitions)
        x = d[, "x"]
        y = d[, "y"]
